...
import { shadcnComponentDefinitions } from './shadcn-definitions';

/**
 * The base vocabulary: what a spec may be made of, before the product page's
 * own components are added on top of it in `catalog.ts`.
 *
...
/**
 * The base catalog, for the renderers in `registry.tsx` to be typed against.
 * It renders no prompt: the catalog a model is handed is `catalog.ts`'s, which
 * extends this vocabulary with the product page's own.
 */
export const baseCatalog = defineCatalog(generativeSchema, { components, actions });

/**
 * The two maps, for the catalog that EXTENDS this vocabulary. Exporting them
