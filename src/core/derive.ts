...
function mapNode(
  ...
  // Version drift: the wire states a kind this build's pinned `mthds` does not
  // define. The switch above is exhaustive over `InputFormItem`, so
  // `satisfies never` keeps the build LOUD when the PIN grows a kind - and
  // this return keeps the mapper TOTAL when a SERVER is ahead of that pin,
