...
/**
 * The rules of a PRODUCT PAGE: what the brand catalog's prompt says beyond
 * the layer's own rules.
 *
 * Every rule of the layer's prompt holds here unchanged but one: the base
 * catalog runs a page from a Button, the brand catalog from its Cta, so that
 * rule is restated for the Cta in its place. The grammar of the page is then
 * appended - the bar first, the hero once with the only h1, the work and the
 * rail inside the workspace, the rail made of bound summary rows ending with
 * the one call to action, the footer last. It is stated as rules rather than
 * guarded by the validator, so a first run measures the prompt: a product-page
 * check is added to the validator only if a run breaks a grammar these rules
 * state.
 */
...
/** The layer\'s rules with the Button rule restated for the Cta, then the grammar of the page. */
