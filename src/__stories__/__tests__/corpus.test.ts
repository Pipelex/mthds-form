...
import { CONTRACTS, OUTPUT_FORM, OUTPUT_SCHEMAS } from '../_generated/results';
...
describe('the structures corpus', () => {
  ...
  it('emits an OUTPUT_FORM and an OUTPUT_SCHEMAS beside every input half', () => {
    // The output half is generated by the same sweep as the input half, so a
    // module carrying only inputs means a stale fixture from before the output
    // simulation existed - and the stories that import it would fail with a
    // missing export rather than with anything that names the cause.
    for (const name of generatedCases()) {
      ...
      expect(source, `${name}.ts has no OUTPUT_SCHEMAS`).toContain('export const OUTPUT_SCHEMAS');
    }
  });
...
describe('the output descriptors', () => {
  ...
  it('carries a payload schema for every pipe it describes', () => {
    // `buildResultField` REQUIRES the schema - it is what deleted the renderer's
    // shape guessing - so a descriptor without one is a fixture the renderer
    // cannot honestly render.
    for (const pipeRef of Object.keys(OUTPUT_FORM)) {
      expect(OUTPUT_SCHEMAS[pipeRef], `${pipeRef} has no output schema`).toBeDefined();
    }
  });
});
