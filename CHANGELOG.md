...
- **`ListRunField.itemCount` carries a declared item count (`Concept[N]`).** Present exactly when the method named a count, absent for a variable `Concept[]` list. Read off the array schema's `minItems` — the keyword ajv itself reads — rather than off the contract's `item_count`, which states the same number.
