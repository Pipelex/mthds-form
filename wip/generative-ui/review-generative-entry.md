...
**Landed.** PR #21 merged to `dev` on 2026-09-05 as `21efb2b`, squashed from `131446f`, with the branch's Quality Checks run green. At landing the merge had not reached `main`; the npm release that carries the entry is a release item's business, sequenced behind the host's Tailwind v4 move. L-260904-c6d660 closed on that merge, and the deferrals stay on L-260905-0420af.
