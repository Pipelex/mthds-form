...
Those invariants are worth the local minute because none of them can fail in a way source review would catch. They walk each entry's built chunk graph, which is where a banned dependency actually arrives: React into the headless `.` entry, or ajv into `./react`, both through a shared chunk rather than through any import statement in that entry's sources. The third checks the `'use client'` prologue esbuild drops and `tsup.config.ts` then re-asserts. All of them are re-checked on `main` by the publish job, but discovering them there means a broken release commit is already merged.
