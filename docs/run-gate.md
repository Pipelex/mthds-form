...
`gateRunInputs` runs `computeReadiness`'s *own* two functions, **and over the same selection**, so the rules are shared by construction rather than by resemblance. (The tree they run over is the gate's own — a private structural walk of the contract's `json_schema` in `gating-fields.ts`, because a server must never need the wire input-form descriptor the browser's render tree is mapped from; agreement between the two trees is what `gate-agreement.test.ts` asserts. That walk follows `$ref` with a per-path cycle guard: a self-referencing model degrades to an opaque leaf instead of overflowing the stack — open by design, because ajv resolves recursion natively and has already enforced *presence* of required children at every depth; presence is all it enforces, though, so a required scalar that arrives present-but-blank below the cycle point is caught by neither side — a recorded residual, not a safety property — while a definition merely referenced from two siblings still expands in full.) The selection is the half that is easy to leave out: sharing `fieldFilled` and `mustBeFilled` but keeping only the inputs the method *demands* still lets a touched optional input through, because the check it would have failed is never run on it. [`src/core/__tests__/gate-agreement.test.ts`](../src/core/__tests__/gate-agreement.test.ts) asserts it by running both sides over one table — including the structured fixtures a host does not have — which is the only form of that claim worth trusting.
...
| `mustBeFilled(field)` | whether a field gates at all — reads the `gating` the wire carries on the render tree, and the one the gate's own tree stamps from the contract |
...
- **A touched *optional* input is held to its concept, on both surfaces.** It is out of the reckoning while untouched and in it the moment something is filled. So the readiness count moves from 3 of 3 to 3 of 4 when the user starts filling an optional structure, and to 4 of 4 when they finish — and the gate applies the same rule to the same inputs, which is not something ajv can do for it: a required child that is a plain string arrives as `''` and satisfies `required`. The rule stated properly is that the gate holds to its concept exactly the inputs it is about to put on the wire, and nothing it will omit — the payload builder tests the very same `isFilled`.
- **A declared item count gates, at both ends.** A `Concept[N]` list is satisfied only by `N` filled items; a variable `Concept[]` list never gates, because its empty form is the legitimate value `[]`. The bounds reach the descriptor as `ListRunField.itemCount` and `maxItemCount` — on the render tree from the wire's `item_count` and the schema's `maxItems`, on the gate's own tree from the same `minItems`/`maxItems` ajv reads — separately, because they are two facts. Too many items is not filled either: the control will not add past the upper bound, so that answers a list which arrived from somewhere else, and calling it *missing* stretches the word a little rather than letting the button offer a run ajv refuses on `maxItems`. Each bound is asked on its own, so a model that states only a ceiling gets one enforced — a nested array inside a structured concept may declare either alone, and a bound the descriptor publishes and nothing reads is a bound only ajv holds.
- **A started list row owes its concept the same as a touched input.** One level down from the rule above, and for the same reason. `isFilled` on an array is `some`, so a row holding `{name: '', notes: 'a thought'}` reads filled while the child its concept requires is blank — and ajv agrees with it, because `''` is a present string. A row the user has put something in must therefore satisfy `fieldFilled` for the item, exactly as a touched optional input must. A row that is empty stays an absence for the purpose of *gating*: an untouched row blocks nothing, and a variable list still never gates on being empty. Note what that does **not** say — an untouched row alongside a filled one still travels, as the shell the bridge built for it (`{name: ''}`). Only a list with no filled row at all deflates away entirely. The asymmetry is deliberate for now and recorded in [`wip/pr-6-review-notes.md`](../wip/pr-6-review-notes.md): dropping empty rows from a mixed list is a change to what goes on the wire, and it has to be decided together with the other absence questions rather than slipped in beside a readiness rule.
...
- **A body that is not an object at all.** `gateRunInputs` takes `unknown` because a public endpoint's argument is whatever was sent. ajv is shown that value rather than a repair of it, which is what makes `must be object` a verdict a caller can actually receive; the indexing that follows uses a safe `{}` stand-in, which is what keeps the gate returning a verdict instead of throwing. Repairing *first* looks equivalent and is not, which is why it was changed: with a contract whose inputs are all optional the `required` list is empty, so the repaired `{}` validated and a string body used to come back `ok: true` on a run that carried no inputs at all. It no longer can — ajv sees the string and answers `must be object` — so a malformed body cannot start a run whatever the contract demands.
- **Cycles and sharing.** The walk keeps the objects it has already judged. That is a cycle guard and a memo at once: an object reached twice within one call answered `false` the first time, since a `true` short-circuits every `some` above it. Without it, a value shaped like a diamond chain costs exponential time. It records the **depth** each object was judged at, because a `false` here has two causes and only one of them travels: past the cap it means "could not look", so a node judged empty at the bottom of a deep branch must be walked again when a shallower path reaches it, or a genuinely filled input reads as missing depending on which branch ran first.
...
[`src/core/__tests__/hostile-values.test.ts`](../src/core/__tests__/hostile-values.test.ts) asserts these through both halves, and [`gate-agreement.test.ts`](../src/core/__tests__/gate-agreement.test.ts) asserts the malformed-body verdicts.
