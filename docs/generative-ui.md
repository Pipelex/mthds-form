...
- _Kind_ — every path the layout **shows** as a value is a scalar. A `SummaryRow` whose `value` read `/inputs/document`, a structure of a url and a filename, was not stale and validated perfectly, and rendered `[object Object]`, because the renderer stringifies what it is handed and only the descriptor knows what kind a path is. A `$state`, an item-relative `$item` or an interpolation of a `$template` sitting in a prop is refused when the path is an `object`, a `list`, a `document` or an `image`; a condition is left alone, since a `$cond` or a `visible` only asks whether a value is set, and so are the props that take a list — every array-typed prop of the catalog, read from its schemas, and the rows a `DataTable` lays out.
