...
The **designer method** is the prompt. It ships as data at `@pipelex/mthds-form/layout-design.mthds`: a `.mthds` bundle whose main pipe, `design_layout`, takes three inputs — `catalog`, `brief` and an optional `seed` — and runs two stages. `plan_page` reads the brief and the catalog and writes a `PagePlan`, a structure the bundle declares: what the page is for, its title and composition, its regions with the elements and paths each holds, what runs the method, the defaults and what is delegated. It alone sees the seed, because the plan is where two runs of the same brief diverge. Neither stage prescribes a composition: the shape of the page is the planner's decision, written into the plan, and the prose names none of the product chrome — what a component is for travels in its catalog description, as data, and a test holds the method to naming none — so two layouts of one brief may differ in their bones and not only in their copy. `emit_page` is handed the plan, the catalog with every props signature and the brief, and emits the JSONL patches. Every paragraph either model reads — the design direction and the planning rules on one side, the output format, the state contract and the numbered rules on the other — is authored prose in that file. It ships as a file rather than as a string baked into a module because it is still being iterated on — a newer method is then a package upgrade rather than a code change — and because nothing in the entry may read it, which is what keeps the entry importable from a browser. Improving the prompt is editing that file; nothing in TypeScript renders a prompt string. Two things to know before editing it: a literal dollar sign is written `$$`, because `$name` is the language's inline substitution, so every json-render expression key in the prose (`$$state`, `$$bindState`, `$$item`, ...) is spelled that way; and the component and action sections, like the plan the builder reads, are Jinja2 loops, so a heading that follows a loop sits directly under its `{% endfor %}` to render one blank line rather than two.
...
- **`make briefs`** rewrites each hero's brief, whose opening line carries the hash of the prompt it was written beside. A third assertion holds those opening lines to the pin, and it is deliberately **not** one of the two the hatch stands down: a brief is free to regenerate and a captured layout is not, so the loop keeps the briefs current rather than excusing them. Skip this step and `generative.test.ts` goes red under the hatch exactly as it does without one. Free and offline.
