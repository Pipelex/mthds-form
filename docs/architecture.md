...
| `file-formats` | what a `document` or `image` slot accepts — a measured mirror of a runtime fact, stamped on each file field as its `formats`, with the helpers (`acceptLabel`, `acceptMap`, `isAcceptedFile`) that take such a list and give the dropzone its hint, its picker filter and its check |
| `narrow-file-formats` | `narrowFileFormats`, which narrows every file slot in a field tree to the MIME types a host's upload path takes, so the form and the host's server hold one list — [docs/upload-seam.md](upload-seam.md) |
...
- **Upload.** Injected through `FieldEnv`, as above. What each side of that seam owes the other — what a slot accepts and how a host narrows it, the ID is a path, an uploading ID means every door into that value is shut, what a row's identity does and does not cover, when `resolveUrl` is not needed — is [upload-seam.md](upload-seam.md).
...
- **Presentation.** `FieldPresentationProvider` picks how a field's label chrome reads, and how an enum value is worded. `studio` (the default) shows the field's name verbatim in mono with its concept pill, because in a builder-facing surface the name IS the identifier written in the `.mthds` file. `app` shows it humanised in sans with no pill, and outlines a field in error rather than only captioning it, because the person filling a published form has never seen the method's source. An authored `title` is authoritative and shown verbatim in both presentations; only the identifier fallback is humanised. A segmented enum control is named for assistive technology by that same label, so a screen reader announces what the page shows. An enum value follows the same switch: in `app` an enum option or result reads as words while the stored value stays the code, and `studio` shows the code. It is the only value the two presentations show differently ([result-view.md](result-view.md)). It is a context rather than a prop deliberately: only the components that own label chrome or an enum value's wording read it — `FieldShell`, `ObjectField`, `BooleanField`, `ListField`, `EnumField` and the result view — plus the file control, whose card replaces a storage reference with the file's format in `app` ([upload-seam.md](upload-seam.md)), and threading a prop would have meant editing every control and both recursive containers to carry something they do not use.
