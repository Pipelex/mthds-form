...
`make storybook` (or `make st`) serves the stories on port 6006, and `make storybook-run` (or `make str`) serves them with the generative study's method pages able to run, taking the hosted API key from your shell (see the generative heroes below); `make build-storybook` produces the static build. The stories also run as tests — `make test` executes them in headless Chromium as a third vitest project — so a story is regression coverage whether or not anyone opens it.
