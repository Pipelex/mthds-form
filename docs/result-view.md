...
## Labels and values follow the presentation, exactly as the form does
...
**Values follow it too, for the reason labels do.** `studio` is a builder's view, where a value has to match the bundle and the JSON the builder will read next. `app` is for a person who has seen neither. So an enum value and a number read differently in the two presentations, and nothing else about a value does:

- **An enum value** is its code in `studio` (`unit_price_differs_from_po`, `HIGH_RISK`) and its code in words in `app` ("Unit price differs from po", "High risk"). `humanizeEnumValue` is `humanizeFieldName` applied after an all-caps code is lowercased, and only a code goes through it: a value holding a space, a slash or a symbol, or one that does not start with a letter (`Very satisfied`, `N/A`, `18-24`), is authored words already and is shown verbatim. Acronyms are not guessed. `po` stays `po` where a reader expects `PO`, exactly as a field name does, and the cure for both is a label the method's author writes, which is a change to the standard's descriptor rather than a guess in the kernel.
- **A number** is printed as it arrived in `studio` and formatted in `app`, as the next paragraph describes.

`LeafValue` owns both rules, so a stacked row, a table cell and a chip cannot disagree. The enum input control follows the same switch: in `app` it offers the words and still stores the code, because a result and the form that produced it show the same fields. Its segmented control measures the label it shows, not the code.

**A number is grouped, with bounded decimals, in `app`.** A payload's number goes through `Intl.NumberFormat` there. An integer only gains grouping (`1200` reads `1,200`), a magnitude of 1 or more keeps at most two decimals (`2116.2` reads `2,116.2`), and a value below 1 keeps three significant digits, so `0.0042` does not read as `0`. It applies wherever a number renders, table cells and chips included. The locale is the `locale` prop on `ResultEnvProvider`, and it defaults to `en-US` rather than to the runtime's own: a page rendered on a server in one locale and hydrated in a browser in another would print `2,116.2` on one side and `2 116,2` on the other, which is a hydration mismatch. The formatters are built once per locale and kept. Three things stay out on purpose. A currency has no word in the intent vocabulary, so an amount is a number and nothing more. The `quantity` and `rating` intents refine this default rather than replace it. And a numeric column is not right-aligned. The same honesty applies to a year held in a `number` field, which reads `2,023` in `app`: telling a year from an amount is a hint's job, not a guess the kernel makes.

**Only the rendering changes.** The JSON view, the copy control and the download carry the payload as it came, byte for byte, which is why none of this could be done by a host rewriting values before rendering them. `Outputs/Readability` shows one real run in both presentations, and `result-values.test.tsx` pins every rule, cells and chips included.
...
**An unbounded `text` value is typeset too.** `text` is the standard's short single-line string, but _short_ is not a property the kind carries (see "Whether a row can be OPENED" above): a `text` field with no `max_length` is a slot a model fills with three sentences, or with a memo holding a heading and a table, and printed plain its `#` and `|` reach the reader. So a `text` field the descriptor does not bound to a cell (`fitsACellWhole` is false: no `max_length` at or below the cell limit) is typeset exactly as `prose` is, in both presentations, and a bounded one stays plain, because its author said it is short. The method does not have to declare anything, and the rule reads a constraint rather than the value, so it is not sniffing. A table cell or a chip keeps the plain string, as it does for `prose`. The accepted risk is a short unbounded value that begins with block syntax: `# `, `- `, `1. ` or four spaces of indentation render as a heading, a list or a code block. Inline markup is typeset as well, so `**bold**` is bold and a bare URL becomes a link.

**A one-line `text` value keeps its markup.** It sits beside its label and ends at the right edge of its column (below), and that layout depends on the value being an inline run inside a box as wide as its content. So `Markdown` takes `loneParagraph="inline"`, which renders a text holding one paragraph as the very span a plain value always was, with the same classes and the same `pre-wrap`, and typesets only its inline markup inside it. A plain one-liner is therefore byte-identical to its rendering before `text` was typeset, which `result-values.test.tsx` pins, and `Outputs/Readability` measures it at the right edge in a browser. `prose` keeps the default, `'block'`, and renders exactly as it did: it never shares a line with its label, so it has no edge to keep.
