...
**A field's description is on hover, at every depth including the top.** On an input it is guidance a person needs before typing; on a result it is a sentence beside a value the reader came for — one line of chrome at the top, ten inside a structure of ten. So it rides the label's `title`, and a **dotted underline** says it is there, because a `title` nobody knows about is a fact nobody reads. One cue in every position — a label, a column header, the output itself — since a cue that appears in some places and not others is worse than none.
