...
| controls (`./react`) | `react` / `react-dom` (optional peers), `react-dropzone`, `lucide-react`, the radix primitives behind the vendored components (`select`, `switch`, `toggle`, `toggle-group`, `tooltip`), `class-variance-authority`, `clsx`, `tailwind-merge`, `mthds` (**types only**, peer) | `ajv`, `ajv-formats`, RJSF, Next.js, `next-intl`, any host import, any _value_ from `mthds`                                  |
