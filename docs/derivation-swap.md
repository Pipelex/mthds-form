...
- **`contentKey`** — the single property a native scalar's value sits inside on the wire (`TextContent {text}`, `NumberContent {number}`, `YesNoContent {yes_no}`). A scalar-kind node (text, prose, number, boolean) whose aligned, dereferenced, nullable-collapsed schema declares exactly one property gets that property's name; anything else gets none. It is a fact about the **payload** shape, which is the schema's job — the descriptor describes the form, never the payload.
