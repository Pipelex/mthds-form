...
The compile-time half is kept by a `satisfies never` after the switch: when the **pinned `mthds`** grows a kind, the build fails and someone maps it deliberately. The runtime return is only ever reached when a **server** is ahead of that pin.
...
5. **The heuristics have no successors.** The depth rule, the url-bearing-object test, the prose-length rule and the schema-`enum` dispatch are deleted — the wire states kinds. Number bounds are no longer read from the schema at all: `min`/`max` come from the wire node, with an exclusive bound converted to the nearest inclusive one on an integer and dropped on a float, never published as the excluded endpoint (ajv still enforces the exact keyword).
